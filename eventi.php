...
// -------------------------
// QUERY EVENTI: SOLO FUTURI + APPROVATI
// + preferenze prima
// + distanza (se geo_ok)
// -------------------------
$where  = [];
...
// ---- pref_sort: 1,2,3... per preferiti, altrimenti 9999
...
if ($geo_ok) {
  ...
  // NOTA: se latitudine/longitudine sono NULL, distanza_km viene NULL
  $distanceSelect = "(
      CASE
      ...
      <?php else: ?>
        <section class="grid" aria-label="Elenco eventi futuri">
          <?php while ($ev = pg_fetch_assoc($res)) : ?>
            ...
            // posti_totali può essere NULL (eventi informativi)
            $hasLimitedSeats = ($ev['posti_totali'] !== null && $ev['posti_totali'] !== '' && (int)$ev['posti_totali'] > 0);
