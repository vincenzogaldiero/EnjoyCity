...
$kpi_eventi_attesa = count_q($conn, "SELECT COUNT(*) c FROM eventi WHERE stato = $1", ['in_attesa']);
$kpi_rec_attesa = count_q($conn, "SELECT COUNT(*) c FROM recensioni WHERE stato = $1", ['in_attesa']);
                    ...
                    <article class="row" data-filter-row>
                        ...
                        <div class="row-actions">
                            <form class="inline" method="post" action="<?= base_url('admin/admin_event_action.php') ?>">
                                ...
                                <button class="btn btn-ok" type="submit" data-confirm="Approvare l’evento #<?= $evId ?>?">Approva</button>
                            </form>

                            <form class="inline" method="post" action="<?= base_url('admin/admin_event_action.php') ?>">
                                <input type="hidden" name="id" value="<?= $evId ?>">
                                <input type="hidden" name="azione" value="rifiuta">
                                <button class="btn btn-danger" type="submit" data-confirm="Rifiutare l’evento #<?= $evId ?>?">Rifiuta</button>
                            </form>
